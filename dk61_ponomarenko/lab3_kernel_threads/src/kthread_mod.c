...
#include <linux/init.h>		// used for kmod_init, kmod_exit macroses
#include <linux/interrupt.h>	// used for interrupts
#include <linux/kthread.h>	// used for kernel threads
#include <linux/list.h>		// used for creating lists
#include <linux/slab.h>		// used for memory allocating
#include <asm/atomic.h>		// used for lock() and unlock() functions

MODULE_DESCRIPTION("Kthread, lists, atomic functions, memory allocating");
MODULE_AUTHOR("bgdpn");
MODULE_VERSION("3.1");
...
MODULE_PARM_DESC(kth_num, "Number of created threads");
MODULE_PARM_DESC(incr_val, "Value by which 'cnt' variable is incremented");
...
{
	while (atomic_cmpxchg(arg, 0, 1) == 1);
}
...
int thread_func(void *data)
{
	...
	struct_Error: printk(KERN_ERR "Kmalloc didn't allocate memory for \
	struct pointer!\n"); 
	do_exit(1);
}
