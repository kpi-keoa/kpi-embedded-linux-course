...
#define MY_DEFINE_SPINLOCK(x) static atomic_t x = ATOMIC_INIT(0);

static void my_spin_lock(atomic_t *tmp)
{
	while(atomic_xchg(tmp, 1));
}

static void my_spin_unlock(atomic_t *tmp)
{
	atomic_set(tmp, 0);
}
