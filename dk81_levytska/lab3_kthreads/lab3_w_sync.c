...
static int __init lab3_no_lock_init(void)
{
	int i;
	...
	if (NULL == thread) {
		pr_err("cannot allocate memory for threads!\n");
		status = -ENOMEM;
		goto init_final;
	}
...
static void __exit lab3_no_lock_exit(void)
{
	int i;
...
module_init(lab3_no_lock_init);
module_exit(lab3_no_lock_exit);