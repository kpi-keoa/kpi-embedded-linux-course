...
static int thread_inccnt = 5;
static unsigned long thread_delay = 40;
...
static unsigned long locks = 0;

module_param(thread_num, int, 0);
MODULE_PARM_DESC(thread_num, "Number of threads");

module_param(thread_inccnt, int, 0);
MODULE_PARM_DESC(thread_inccnt, "How many  incrementations of glob_var");

module_param(thread_delay, ulong, 0);
MODULE_PARM_DESC(thread_delay, "Delay between  incrementations");

static void lock(unsigned long *l, unsigned long lock_num)
{
	while (test_and_set_bit(lock_num, l)) {
		schedule();
	}
}

static void unlock(unsigned long *l, unsigned long lock_num)
{
	clear_bit(lock_num, l);
}
...
static int __init lab3_own_sync_init(void)
{
	int i;
	...
	if (thread_num <= 0) {
		pr_err(" invalid number of threads!\n");
		goto init_final;
	}
	if (thread_inccnt <= 0 ) {
		pr_err(" invalid number of iterations!\n");
		goto init_final;
	}
	if (thread_delay < 0 ) {
		pr_err(" invalid delay value!\n");
		goto init_final;
	}

	thread = kmalloc(thread_num*sizeof(**thread), GFP_KERNEL);
	if (NULL == thread) {
		pr_err(" not possible to allocate memory for threads!\n");
		status = -ENOMEM;
		goto init_final;
	}
	...
	for (i = 0; i < thread_num; i++) {
		...
		if (IS_ERR(thread[i])) {
			pr_err("thread-%d was not created!\n", i);
			thread[i] = NULL;
		} else {
...
static void __exit lab3_own_sync_exit(void)
{
	int i;
...
exit_final:
	pr_info("Modue %s  removed\n", module_name(THIS_MODULE));

}

module_init(lab3_own_sync_init);
module_exit(lab3_own_sync_exit);