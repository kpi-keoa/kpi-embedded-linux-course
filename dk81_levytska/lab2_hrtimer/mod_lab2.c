...
static int __init mymodule_init(void)
{
	...
	printk(KERN_INFO "mod_lab2: module: jiffies = %lu\n", jiffies);
	tasklet_schedule(&my_tasklet);

	if (0 == cnt) {
		pr_warn("mod_lab2: cnt = 0\n");
		return 0;
	}
	...
	if (NULL == array) {
		pr_err("mod_lab2: array = NULL\n");
		return 0;
	}

	if (0 == delay)
		pr_warn("mod_lab2: delay = 0\n");
...
static void my_tasklet_handler(struct tasklet_struct *data)
{
	pr_info("mod_lab2: tasklet: jiffies = %lu\n", jiffies);
}
...
static void __exit mymodule_exit(void)
{
	...
	if (hrtimer_is_queued(&test_hrtimer)) {
		pr_warn("mod_lab2: timer is queued\n");
		pr_info("mod_lab2: I killed test_timer\n");
		hrtimer_cancel(&test_hrtimer);
	}

	for (i = 0; i < ticks; i++)
		pr_info("mod_lab2: timer: array[%lu] jiffies= %lu\n", i, array[i]);
